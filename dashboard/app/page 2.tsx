...
import { Ship, Activity, Bell, LayoutDashboard, AlertTriangle, Moon, Sun } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const mockIncidents = generateMockIncidents(mockVessels, 8);

    setVessels(mockVessels);
    setIncidents(mockIncidents);
    setStats(calculateFleetStats(mockVessels, mockIncidents));
  }, []);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-2">Average Fuel Level</h3>
            <div className="flex items-end space-x-2">
              <p className="text-3xl font-bold text-gray-900">{stats.averageFuelLevel}%</p>
              <p className="text-sm text-gray-500 mb-1">across fleet</p>
            </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-2">Critical Incidents</h3>
            <div className="flex items-end space-x-2">
              <p className="text-3xl font-bold text-red-600">{stats.criticalIncidents}</p>
              <p className="text-sm text-gray-500 mb-1">severity 4-5</p>
            </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-2">Vessel Distribution</h3>
            <div className="space-y-2 mt-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Underway</span>
                <span className="font-medium text-gray-900">{stats.activeVessels}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">At Anchor</span>
                <span className="font-medium text-gray-900">{stats.anchoredVessels}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Offline</span>
                <span className="font-medium text-gray-900">{stats.offlineVessels}</span>
              </div>
            </div>
          </div>
        </div>
